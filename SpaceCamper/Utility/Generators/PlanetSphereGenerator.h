 ...
 */
class /*GEOMETRYCORE_API*/ FPlanetSphereGenerator : public FMeshShapeGenerator
{
public:
	/** Radius */
	double Radius = 1;
...
private:
	...
	void GenerateUVVertices()
	{
		...
		int32 NorthPlane = FMath::RoundToInt32(static_cast<float>((NumPhi - 1) / 10)); // 북극에서부터 10% 아래쪽
		int32 SouthPlane = FMath::RoundToInt32(static_cast<float>((NumPhi - 1) / 10)); // 남극에서부터 10% 위쪽
		float NorthVal = UVPhi + DUVphi * (NumPhi - 3 - NorthPlane);

		for (p = 1, UVPhi = NorthVal; p <= 1 + NorthPlane; ++p)
		{
			for (t = 0, UVTheta = 1; t < NumTheta; ++t, ++UVIdx, UVTheta += DUVtheta)
			{
				UVs[UVIdx] = FVector2f(0.f, NorthVal);
				UVParentVertex[UVIdx] = (p - 1) * NumTheta + t;
			}
			UVs[UVIdx] = FVector2f(0.f, NorthVal);
			UVParentVertex[UVIdx] = (p - 1) * NumTheta; // Wrap around
		...
		for (; p < NumPhi - 1 - SouthPlane; ++p, UVPhi += DUVphi)
		{
			for (t = 0, UVTheta = 1; t < NumTheta; ++t, ++UVIdx, UVTheta += DUVtheta)
			{
				UVs[UVIdx] = FVector2f(UVTheta, UVPhi);
		...
		float SouthVal = UVPhi -= DUVphi;
		for (; p < NumPhi - 1; ++p)
		{
			for (t = 0, UVTheta = 1; t < NumTheta; ++t, ++UVIdx, UVTheta += DUVtheta)
			{
				UVs[UVIdx] = FVector2f(0.f, SouthVal);
				UVParentVertex[UVIdx] = (p - 1) * NumTheta + t;
			}
			UVs[UVIdx] = FVector2f(0.f, SouthVal);
			UVParentVertex[UVIdx] = (p - 1) * NumTheta; // Wrap around
